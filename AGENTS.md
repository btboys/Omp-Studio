...
- Current release: `0.5.12` (package.json `version`); pinned omp runtime `17.2.15` (`ompRuntimeVersion`).
...
**IPC surface:** invoke namespaces `app:`, `thread:`, `plugins:`, `automation:`, `settings:`, `window:` (e.g. `thread:prompt`, `thread:setPermission`, `app:getConfig`, `settings:saveModels`, `plugins:getPackages`, `automation:runNow`, `window:close`). Push channels: `pi:event`, `pi:extui`, `pi:exit`, `pi:error`, `pi:automation`, `pi:appUpdate`, `pi:coreUpdate`, `pi:notify-activate`, `window:maximized-changed`.

**Desktop notifications:** built-in OS notifications (`src/main/notify.ts`) fire on `agent_settled`, Sandbox/extui approval prompts, and unexpected process errors. Preferences live in `AppConfig.desktopNotify` (Settings → 诊断与配置). Multi-tab aware: bodies include `project · session title`; `onlyWhenUnfocused` suppresses only the *active* tab while the window is focused (background tabs still notify). Renderer syncs the active tab via `app:setActiveThread`. Clicking a notification focuses the window and opens/switches that tab via `pi:notify-activate` → `goToThread` / `setActiveThread`.
