...
const exact: Record<string, string> = {
  ...
  "归档只会从侧栏、搜索和新建任务的项目列表中隐藏文件夹，不会删除文件夹或其中的线程；在「已归档项目」中可彻底删除会话文件，不可恢复。": "Archiving only hides the folder from the sidebar, search, and new-task project list. It does not delete the folder or its threads; archived projects can be permanently deleted (irreversible).",
  ...
  "图片预览": "Image preview",
  "终端 omp 的 Windows 桌面端：完整继承模型、harness 与插件系统。左侧选择项目与线程，右侧预览文件。": "A Windows desktop client for terminal omp, with its models, harness, and extension system. Choose projects and threads on the left and preview files on the right.",
  ...
  "触发时在该文件夹新建一个 omp 会话执行，完成后保存为可查看的线程。": "When triggered, a new omp session runs in this folder and is saved as a viewable thread.",
  "重复": "Repeat",
  "按小时": "Hourly",
  "每天": "Daily",
  "每周": "Weekly",
  ...
  "原地更新）。点击下方按钮后，Omp Studio 会自行下载并安装新版本到应用数据目录，更新完成后新开的线程使用新版本。扩展请在「插件」面板更新。": ". Use the button below to download and install a new version into app data. New threads use it after the update. Update extensions in the Plugins panel.",
  "运行": "Run",
  "更新 omp 本体（不含扩展，扩展请在「插件」面板更新）。会先检查是否为最新版本，结果以提示呈现。更新完成后新开的线程使用新版本。": "to update the omp CLI itself. Extensions are updated separately in the Plugins panel. omp checks the current version first, and new threads use the updated version.",
};
